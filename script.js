...
function checkLevel() {
    ...
    if (newLevel !== level) {
        ...
        if (newLevel > oldLevel) {
            ...
            if (newLevel === 2) { addTrees(2); }
            if (newLevel === 3) { addTrees(2); addRecyclingBins(2); }
            if (newLevel === 4) { addTrees(3); addRecyclingBins(2); addParks(1); }
            if (newLevel === 5) { addTrees(5); addRecyclingBins(3); addParks(2); }
...
function npcSpeak(npcID, text) {
    ...
    setTimeout(function() { npc.speech.style.display = "none"; }, 4500);
}
...
function moveNPC(npcID, type) {
    ...
    setTimeout(function() { npcSpeak(npcID, activity.message); }, 6000);
}
...
if (npc1) { npc1.addEventListener("click", function() { npcSpeak("npc1", "I love cycling around town!"); }); }

const npc2 = document.getElementById("npc2");
if (npc2) { npc2.addEventListener("click", function() { npcSpeak("npc2", "Remember to recycle!"); }); }

const npc3 = document.getElementById("npc3");
if (npc3) { npc3.addEventListener("click", function() { npcSpeak("npc3", "Look at those trees!"); }); }
...
function startMiniGame(type) {
    ...
    if (!overlay) {
        console.error("Mini-game HTML not found!");
    ...
    const seedSpawner = setInterval(function() {
        ...
        const fall = setInterval(function() {
            ...
            if (seedRect.bottom >= basketRect.top && seedRect.left < basketRect.right && seedRect.right > basketRect.left) {
                ...
                if (seed.innerText.includes("✨")) { caught += 2; } else { caught++; }
                miniScore = caught;
...
function startSolarGame() {
    ...
    function createEnergy() {
        ...
        object.style.transition = "transform 0.1s";
        ...
        object.onclick = function() {
            ...
            if (score >= target) {
                ...
                setTimeout(function() { miniGameWin(); }, 600);
            }
        };

        area.appendChild(object);
        setTimeout(function() { if (object && object.parentNode) { object.remove(); } }, 2200);
    }
...
function startRecycleGame() {
    ...
    binData.forEach(function(data) {
        ...
        bin.onclick = function() {
            ...
            if (selectedItem.type === data.type) {
                ...
                createPopEffect(selectedItem.element);
                ...
                if (score >= target) {
                    ...
                    setTimeout(function() { miniGameWin(); }, 700);
                }
            } else {
                combo = 0;
                comboDisplay.innerText = "🔥 Combo: 0";
                document.getElementById("miniGameMessage").innerText = "❌ That's the wrong bin!";
                selectedItem.element.style.transform = "translateX(-8px)";
                setTimeout(function() { if (selectedItem && selectedItem.element) { selectedItem.element.style.transform = ""; } }, 120);
            }
        };
    ...
    const rubbish = [
        { icon: "📰", type: "paper" }, { icon: "📦", type: "paper" },
        { icon: "🥤", type: "plastic" }, { icon: "🧴", type: "plastic" },
        { icon: "🍎", type: "food" }, { icon: "🍌", type: "food" },
        { icon: "🔋", type: "electronic" }, { icon: "📱", type: "electronic" }
    ];
    ...
    function spawnRubbish() {
        ...
        item.onclick = function() {
            if (selectedItem === item) {
                ...
                document.getElementById("miniGameMessage").innerText = "↩️ Put it down!";
                return;
            }
        ...
        setTimeout(function() { if (item.parentNode && selectedItem?.element !== item) { item.remove(); } }, 4500);
    }
    ...
    function createPopEffect(element) {
        ...
        const animation = setInterval(function() {
            ...
            effect.style.opacity = 1 + y / 40;
            if (y <= -40) { clearInterval(animation); effect.remove(); }
        }, 30);
    }
        ...
        const fall = setInterval(function() {
            ...
            if (bikeRect.left < objectRect.right && bikeRect.right > objectRect.left && bikeRect.top < objectRect.bottom && bikeRect.bottom > objectRect.top) {
                ...
                if (object.dataset.type === "package") {
                    ...
                    if (score >= target) { clearInterval(spawn); miniGameWin(); }
                } else {
                    score = Math.max(0, score - 1); miniScore = score;
                    document.getElementById("miniScore").innerText = score;
                    document.getElementById("miniGameMessage").innerText = "💥 Ouch! Watch out!";
                }
            }
            if (objectY > area.clientHeight) { clearInterval(fall); object.remove(); }
        }, 30);
    }, 800);
}
...
function startBusGame() {
    ...
    const spawn = setInterval(function() {
        ...
        if (random < 0.55) { object.innerText = Math.random() < 0.5 ? "🌱" : "🪙"; object.dataset.good = "true"; }
        else if (random < 0.85) { object.innerText = "🚗"; object.dataset.good = "false"; }
        else { object.innerText = "🚧"; object.dataset.good = "false"; }
        ...
        const fall = setInterval(function() {
            ...
            if (busRect.left < objectRect.right && busRect.right > objectRect.left && busRect.top < objectRect.bottom && busRect.bottom > objectRect.top) {
                ...
                if (object.dataset.good === "true") {
                    score++; miniScore = score;
                    document.getElementById("miniScore").innerText = score;
                    document.getElementById("miniGameMessage").innerText = "🌱 Great driving!";
                    if (score >= target) { clearInterval(spawn); miniGameWin(); }
                } else {
                    score = Math.max(0, score - 1); miniScore = score;
                    document.getElementById("miniScore").innerText = score;
                    document.getElementById("miniGameMessage").innerText = "💥 Watch out!";
                }
            }
            if (objectY > area.clientHeight) { clearInterval(fall); object.remove(); }
        }, 30);
...
function startHouseGame() {
    ...
    function showAppliance() {
        if (!miniGameRunning || currentMiniGame !== "house") { gameActive = false; return; }

        area.innerHTML = "";
        const appliance = appliances[Math.floor(Math.random() * appliances.length)];
        const item = document.createElement("button");

        item.className = "house-energy-item";
        item.innerText = appliance.icon;
        item.title = appliance.name;
        item.style.position = "absolute";
        ...
        item.onclick = function() {
            ...
            if (appliance.waste) {
                score++; miniScore = score;
                document.getElementById("miniScore").innerText = score;
                document.getElementById("miniGameMessage").innerText = "⚡ Good! Energy saved!";
                item.style.transform = "scale(1.5)";
                if (score >= target) { gameActive = false; miniGameWin(); return; }
                setTimeout(showAppliance, 300);
            } else {
                score = Math.max(0, score - 1); miniScore = score;
                document.getElementById("miniScore").innerText = score;
                document.getElementById("miniGameMessage").innerText = "⚠️ This appliance should stay on!";
        ...
        const movement = setInterval(function() {
            if (!gameActive || !miniGameRunning) { clearInterval(movement); return; }
            position += direction * 0.25;
            if (position > 82) { direction = -1; }
        ...
        setTimeout(function() {
            ...
            if (area.contains(item)) {
                item.remove();
                document.getElementById("miniGameMessage").innerText = "⏰ Too slow!";
            }
            showAppliance();
        }, disappearTime);
    }
...
function miniGameLose() {
    ...
    const message = document.getElementById("miniGameMessage");
    if (message) { message.innerText = "⏰ TIME'S UP!"; }
